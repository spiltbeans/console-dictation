...
const dictator = {
	...
	config: ({ paths = {}, log_names = {}, indexing_config = {} } = {}) => {
		...
		if (Object.keys(paths).length !== Object.keys(log_names).length) throw new Error("Could not complete config. Path and Log Name keys mismatch")

		const { isValid, message } = isRequestValid(log_names, Object.keys(paths))

		if (!isValid) throw new Error(`Could not complete config. Path and Log Name keys mismatch. ${message}`)
